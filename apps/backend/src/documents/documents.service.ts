...
export class DocumentsService {
  ...
  async update(
    ...
    // დავაზღვიოთ hazards-თვის id გენერაცია, თუ ახალი ელემენტებია უid-ოდ
    const ensureHazardIds = (hazards: any[]): any[] => {
      return hazards.map((h: any) => ({
        ...h,
        id:
          h?.id ||
          `hazard_${Date.now()}_${Math.random().toString(36).slice(2)}`,
      }));
    };
    ...
    Object.keys(updateDocumentDto).forEach((key) => {
      ...
      if (value !== undefined) {
        if (key === 'hazards' && Array.isArray(value)) {
          (updateData as any)[key] = ensureHazardIds(value);
        } else {

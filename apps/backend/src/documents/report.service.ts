...
export class ReportService {
  ...
  async generateExcelReport(document: any): Promise<Buffer> {
    ...
    // Document-level planned review date/time (if present)
    const plannedReview = processedDocument.reviewDate
      ? new Date(processedDocument.reviewDate)
      : null;
    const plannedReviewStr = plannedReview
      ? `${plannedReview.toLocaleDateString('ka-GE')}\n${plannedReview.toLocaleTimeString('ka-GE', { hour12: false, hour: '2-digit', minute: '2-digit' })}`
      : '';
    ...
    const headerData = [
      ...
      [
        ...
        '',
        '',
      ], // A5: document-level planned review
    ];
    ...
    for (let i = 2; i <= headerData.length + 1; i++) {
      ...
      row.eachCell((cell, colNumber) => {
        ...
        } else if (i === 5 && colNumber === 1) {
          // Row 5 label at A (planned review)
          cell.font = { bold: true, size: 10, name: 'Arial' };
          cell.fill = {
            type: 'pattern',
            pattern: 'solid',
    ...
    // 10.1. ჩასვათ პატარა თამბნეილი (პირველი ფოტო) მთავარ ცხრილში, სვეტში C
    try {
      const hazardsForThumbs = Array.isArray(processedDocument.hazards)
        ? processedDocument.hazards
        : [];
      hazardsForThumbs.forEach((hazard: any, idx: number) => {
        if (
          !hazard ||
          ...
          return;
        const photo: string = hazard.photos[0];
        ...
        try {
          ...
          const imageId = (workbook as any).addImage({
            base64: base64Data,
            extension,
          });
          const targetRow = dataStartRow + idx;
          // Column C is index 2 in zero-based tl.col
          (worksheet as any).addImage(imageId, {
            tl: { col: 2, row: targetRow - 1 },
            ext: { width: 100, height: 75 },
          });
          // Increase row height to fit thumbnail
          const r = worksheet.getRow(targetRow);
          if (!r.height || r.height < 60) r.height = 60;
        } catch {}
      });
    } catch {}

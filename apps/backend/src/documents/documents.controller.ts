...
export class DocumentsController {
  ...
  async create(
    ...
    try {
      ...
      if (createDocumentDto.hazards && createDocumentDto.hazards.length > 0) {
        console.log(
          '📥 FIRST HAZARD:',
          JSON.stringify(createDocumentDto.hazards[0], null, 2),
        );
      }
      ...
      console.log('📝 Received document data:', {
        ...
        hazardPhotosCount: Array.isArray(createDocumentDto.hazards)
          ? createDocumentDto.hazards.reduce(
              (acc: number, h: any) => acc + (h.photos?.length || 0),
              0,
            )
          : 0,
      ...
      if (
        !createDocumentDto.evaluatorName ||
        !createDocumentDto.evaluatorLastName ||
        !createDocumentDto.objectName ||
        !createDocumentDto.workDescription
      ) {
      ...
      console.log('✅ Final document data:', {
        hazardsCount: Array.isArray(createDocumentDto.hazards)
          ? createDocumentDto.hazards.length
          : 0,
        photosCount: Array.isArray(createDocumentDto.photos)
          ? createDocumentDto.photos.length
          : 0,
      });
  ...
  async downloadDocument(
    ...
    try {
      ...
      try {
        ...
        console.warn(
          '⚠️ Failed to increment ZIP download counter:',
          e?.message,
        );
      }
  ...
  async downloadPhotos(
    ...
    try {
      ...
      try {
        ...
        console.warn(
          '⚠️ Failed to increment ZIP download counter (photos):',
          e?.message,
        );
      }
      ...
      res
        .status(500)
        .json({ message: 'Photos download failed', error: error.message });
    }
  }
  ...
  async downloadExcelReport(
    ...
    try {
      ...
      const excelBuffer =
        await this.reportService.generateExcelReport(documentWithUserInfo);
      // increment Excel download counter
      try {
        await this.documentsService.incrementDownloadCounter(id, 'excel');
      } catch (e) {
        console.warn(
          '⚠️ Failed to increment Excel download counter:',
          e?.message,
        );
      }
      ...
      res.status(500).json({
        message: 'Excel რეპორტის გენერაცია ვერ მოხერხდა',
        error: error.message,
      });
    }
  }
  ...
  async downloadPDFReport(@Param('id') id: string, @Res() res: Response) {
    try {
      ...
      try {
        ...
        console.warn(
          '⚠️ Failed to increment PDF download counter:',
          e?.message,
        );
      }
      ...
      res.status(500).json({
        message: 'PDF რეპორტის გენერაცია ვერ მოხერხდა',
        error: error.message,
      });
    }
  }
  ...
  async getPDFDiagnostics(@Res() res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: 'დიაგნოსტიკის შეცდომა', error: error.message });
    }
  }
  ...
  async generateExcelFromData(@Body() documentData: any, @Res() res: Response) {
    try {
      ...
      if (documentData.risks && !documentData.hazards) {
        ...
        console.log(
          '🔄 Converted risks to hazards format for Excel generation',
        );
      }

      const excelBuffer =
        await this.reportService.generateExcelReport(processedData);
      ...
      console.log(
        `✅ Excel რეპორტი გენერირდა წარმატებით, ზომა: ${excelBuffer.length} bytes`,
      );
    } catch (error) {
  ...
  async getPhoto(
    ...
    const base64Photo = await this.documentsService.getPhoto(
      id,
      parseInt(photoIndex),
    );
